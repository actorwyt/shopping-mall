...
<body> 
	...
	<script type="text/javascript">
		$(function(){			
			...
			var url = "${contextPath}/shoppingCart/countCartGoods?status=identical";
		...
		function changePage(currentPage) {
			...
			$.ajax({
				type: "GET",
				dataType: "json",
				url:"${contextPath}/shoppingCart/getCartGoods",
				data:$.param(params),
                	...
                	for(var i = 0; i < data.length ; i ++) {
						...
						list += '</a></li>'	
							 +	'<li><span class="name">' + data[i].goodsName + '</span></li>'
 							 ...
 							 +  '<li><span class="amount">' + data[i].unitPrice + '</span></li>'
		...
		function deleteGoods(target, goodsId) {
            Messager.confirm({ message: "确认要删除选中的商品吗？" }).on(function (e) {
                if (!e) {
                    return;
                }
    			$.ajax({
    				url:"${contextPath}/shoppingCart/deleteGoods",
    				type:"post",
    				dataType:"json",
		...
		function checkout() {
			...
			Messager.confirm({ message: "确认" + confirmMsg }).on(function (e) {
				...
				$.ajax({
					type: "POST",
					dataType: "json",
					url:"${contextPath}/shoppingCart/checkout",
